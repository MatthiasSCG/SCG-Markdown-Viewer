...
## Angestrebte Angestrebte Lösungsansatz
...
- **Settings-Migration** (electron-store):
  ...
  - Angestrebte Lösung: Im Main-Prozess vor `new Store(...)` prüfen, ob `<userData>/SCG Markdown/config.json` existiert. Falls **nicht**, prüfen, ob `<userData>/Markdown Viewer/config.json` existiert. Falls ja: Inhalt einlesen, ins neue Verzeichnis kopieren, Hinweis in `console.log` ausgeben („Settings aus Vorgängerinstallation migriert").
