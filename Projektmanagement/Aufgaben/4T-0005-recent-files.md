...
- **Klick-Verhalten**: Klick auf einen Recent-Eintrag öffnet die Datei als **neuen Tab im aktiven Fenster** (oder aktiviert den bestehenden Tab, wenn die Datei dort schon offen ist). Die Recent-Liste rutscht entsprechend nach vorne. Damit verhält sich Recent-Klick analog zu „Öffnen mit" im Explorer; konsistent dazu sind auch `Datei → Neu` und `Datei → Öffnen…` (4T-0006) als Tab-Öffner im aktiven Fenster umgesetzt (Spec-Revision während der Klärung, siehe dort).
...
**Verhaltens-Korrektur gegenüber Ursprungsplan**: Klick auf einen Recent-Eintrag öffnet die Datei als **neuen Tab im aktuellen Fenster**, nicht als neues Fenster. So verhält sich Recent analog zu „Öffnen mit" im Explorer. Diese Korrektur wurde anschließend auch auf `Datei → Neu` und `Datei → Öffnen…` ausgedehnt (siehe 4T-0006), sodass alle drei Befehle einheitlich Tabs im aktiven Fenster eröffnen — sauberer und intuitiver als die ursprünglich gedachte Mischung aus „Recent öffnet Tab" und „Neu/Öffnen öffnen Fenster".
