...
- **Pflicht-Check vor jedem `npm run build`**: Vor dem ersten Build eines neuen Epics zwingend prüfen, ob `package.json` bereits auf die **Zielversion** der laufenden Entwicklung gebumpt ist (also nicht mehr die Vorgängerversion trägt). Wenn der Bump vergessen wird, baut `electron-builder` die EXEs mit dem alten Versionsnamen, der `postbuild`-Hook verschiebt sie nach `releases/` und **überschreibt** dort die offiziellen EXEs der Vorgängerversion. Wiederherstellung anschließend nur per `gh release download v<x.y.z>` aus dem GitHub-Asset (umständlich und fehleranfällig wegen Datei-Namen-Konvertierung Punkt ↔ Leerzeichen). Aufgetreten in 4T-0049 (überschrieb 0.15.0) und 4T-0054 (überschrieb 0.16.0). **Konkret als erster Schritt jedes neuen Tasks im Epic: `grep version package.json` zeigt die Zielversion. Wenn nicht: bump und committen, bevor der erste Build läuft.**
